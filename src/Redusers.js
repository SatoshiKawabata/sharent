...
import FileListUtil from './utils/FileListUtil';
...
export default function Redusers(state = initialState, action) {
  ...
  switch (action.type) {
    ...
    case ACTIONS.ON_FILES_LIST_FOLDER:
      newState.files = FileListUtil.sortByDate(action.res.entries);
      // newState.files = action.res.entries;
      newState.pathHistory = [ ...state.pathHistory, action.path ];
    ...
    case ACTIONS.HISTORY_BACK:
      newState.files = FileListUtil.sortByDate(action.res.entries);
      // newState.files = action.res.entries;
      newState.pathHistory = [ ...action.pathHistory ];
      newState.loading = false;
      return newState;
